...
class RelationItem
{
    ...
    public function add(relationType : Int, usePercent : Bool) : Void
    {
        ...
        info.affectBySelfSizeChanged = relationType >= RelationType.Center_Center && relationType <= RelationType.Right_Right || relationType >= RelationType.Middle_Middle && relationType <= RelationType.Bottom_Bottom;
    ...
    @:allow(fairygui)
    private function quickAdd(relationType : Int, usePercent : Bool) : Void
    {
        if (relationType == RelationType.Size) 
        {
    ...
    public function copyFrom(source : RelationItem) : Void
    {
        ...
        _defs.length = 0;
        for (info/* AS3HX WARNING could not determine type for var: info exp: EField(EIdent(source),_defs) type: null */ in source._defs)
        {
    ...
    public function applyOnSelfResized(dWidth : Float, dHeight : Float) : Void
    {
        ...
        if (ox != _owner.x || oy != _owner.y) 
        {
            ...
            {
                for (trans/* AS3HX WARNING could not determine type for var: trans exp: EField(EField(EIdent(_owner),parent),_transitions) type: null */ in _owner.parent._transitions)
                {
...
class RelationDef
{
    public var affectBySelfSizeChanged : Bool;
